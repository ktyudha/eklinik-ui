...
import useDeleteMenu from "@/services/admin/menu/hooks/useDeleteMenu";
...
const DeleteMenuModal: FunctionComponent<Props> = ({
  id,
  name,
  onOpen,
  onClose,
  ...
  const onDelete = async () => {
    setIsSubmit(true);
    const { error, response } = await deleteMenu(id);
    if (error || response) {
      if (error) {
        toast.error("Gagal Menghapus Grup Pertanyaan", {
          position: toast.POSITION.TOP_CENTER,
        });
      } else {
        toast.success("Sukses Menghapus Grup Pertanyaan", {
          position: toast.POSITION.TOP_CENTER,
        });
