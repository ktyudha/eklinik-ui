...
import useCreateMenu from "@/services/admin/menu/hooks/useCreateMenu";
...
const CreateMedicineCategoryModal: FunctionComponent<Props> = ({
  ...
  const onSubmit: SubmitHandler<FormFields> = async (state) => {
    const { error, response } = await createMenu({ ...state });
    if (error || response) {
      if (error) {
        toast.error("Gagal Menambahkan Grup Pertanyaan", {
          position: toast.POSITION.TOP_CENTER,
        });
      } else {
        toast.success("Sukses Menambahkan Grup Pertanyaan", {
          position: toast.POSITION.TOP_CENTER,
        });
